...
对博客的样式进行了相应改动（只针对light模式... 所以索性把导航栏中的`颜色模式`去掉了 - 代码在`\node_modules\vuepress-theme-reco\components\Navbar.vue`中，注释掉`<!-- <Mode /> -->`即可）
...
//.sidebar-group > ul > li > a.sidebar-link.active{
...
.sidebar-sub-headers > li > .sidebar-sub-headers > li > a.sidebar-link{
	font-size: 0.8rem;
	color: #9DAAB6;
}
a.sidebar-link{
